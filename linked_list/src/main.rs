...
impl<T> List<T> {
    ...
    // 删除head节点
    // head指向下一个节点
    // 如果节点为最后一个节点, tail还要置空
    pub fn pop_front(&mut self) -> Option<T> {
        // 可以使用map() -> Option 代替if let
        if let Some(old_head) = self.head.take() {
            ...
            return Some(Rc::try_unwrap(old_head).ok().unwrap().into_inner().value);
        }
        None
    }
